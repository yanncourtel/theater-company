...
class StatementPrinter {
  def print(invoice: Invoice, plays: Map[String, Play]): String = {
    ...
    totalAmount = calculateTva(totalAmount, invoice.customer.substring(0, 2)).toInt

    result += s"Amount owed is ${NumberFormat.getCurrencyInstance(culture).format(totalAmount)}$lineSeparator"
    result += s"You earned ${volumeCredits} credits$lineSeparator"

    result
  }

  private def calculateTva(amount: Int, country: String) = amount * (if (country.equals("FR")) 1.055 else 1)
}
